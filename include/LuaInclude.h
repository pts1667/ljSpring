...
static inline int luaL_checkboolean(lua_State* L, int idx)
{
...
{
	return luaL_opt(L, luaL_checkboolean, idx, def) != 0;
}

// LuaJIT uses int bools
static inline bool lua_toboolean_bool(lua_State *L, int idx)
{
...
#define luaL_optboolean(L,idx,def)     (luaL_SpringOpt<bool>(L,idx,def,::luaL_optboolean,lua_toboolean_bool,LUA_TBOOLEAN,__FUNCTION__))
